...
function init() {
    ...
    */
    ...
    // Создаем экранный счетчик очков 
...
// Функция для обновления счетчика экрана

function scoreUdate(score) {
    var scoreBoard = document.getElementById('score');
    scoreBoard.innerHTML = '<h1>Ваш счет: ' + score + '</>';
}
...
function createBomb() {
    ...
    while (!bombCreated) { //пока бомбу не создали
        ...
        // проверка на змейку
        if ((!bomb_cell_classes.includes('snake-unit')) && (!bomb_cell_classes.includes('bomb-unit'))) {
            var classes = '';
